...
export default function PipelineControlPanel() {
  const [stages, setStages] = useState(
    PIPELINE_STAGES.reduce(
      (acc, s) => ({ ...acc, [s.id]: s.defaultActive }),
      {}
    )
  );
      ...
      <div className="pipeline-header">
        <div className="pipeline-header-left">
          ...
          <span className="pipeline-count">
            {activeCount}/{PIPELINE_STAGES.length} active
          </span>
        </div>
        <div className="pipeline-header-right">
          <button
            className={`btn pipeline-toggle-btn ${
          ...
          return (
            <div key={stage.id} className="pipeline-stage-wrap">
              ...
              >
                ...
                {/* Toggle */}
                <div
                  className={`pipeline-stage-toggle ${
                    isEnabled ? "on" : "off"
                  }`}
              ...
              {idx < PIPELINE_STAGES.length - 1 && (
                <div
                  className={`pipeline-connector ${isActive ? "active" : ""}`}
                >
                  <div
                    className="pipeline-connector-line"
                    style={isActive ? { background: stage.color } : {}}
                  />
                </div>
              )}
            </div>
      ...
      <div className="pipeline-status-bar">
        {PIPELINE_STAGES.map((stage) => (
          <div
            ...
            style={{
              background:
                stages[stage.id] && pipelineRunning
                  ? stage.color
                  : "var(--border-light)",
