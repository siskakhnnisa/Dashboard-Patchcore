  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      ...
      setAnomalyScore((prev) =>
        Math.min(1, Math.max(0, prev + (Math.random() - 0.48) * 0.03))
      );
      setArea((prev) =>
        Math.round(
          Math.max(800, Math.min(5000, prev + (Math.random() - 0.5) * 150))
        )
      );
      ...
      {hasDetections && (
        <div className="det-info-footer">
          ...
          <span className="det-info-footer-severity">
            {detections.filter((d) => d.severity === "high").length} high
            severity
          </span>
        </div>
      )}
