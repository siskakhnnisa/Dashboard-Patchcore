...
import { AlertTriangle, Info, CheckCircle, Clock, ChevronDown } from 'lucide-react'
...
const INITIAL_EVENTS = [
  ...
  },
  {
    ...
    details: 'Object: Unidentified debris | Area: 890 px | Persistence: 4 frames',
    ...
    if (liveDetections.length > 0) {
      ...
      if (highSev.length > 0) {
        setEvents(prev => {
          ...
          const last = prev[0]
          if (last && new Date() - last.time < 30000 && last.type === 'detection') return prev
          const newEvent = {
            id: Date.now(),
            type: 'detection',
            message: `FOD detected — ${highSev.length} object${highSev.length > 1 ? 's' : ''}`,
    ...
    <div className="timeline card">
      <div className="timeline-header">
        <div className="timeline-title-row">
          <span className="section-title" style={{ margin: 0 }}>FOD Events Timeline</span>
      ...
      <div className="timeline-list">
        {events.map(event => {
          ...
          return (
            <div
              key={event.id}
              className={`timeline-event ${event.type} ${isExpanded ? 'expanded' : ''}`}
              onClick={() => setExpandedId(isExpanded ? null : event.id)}
            >
              {/* Timeline line */}
              <div className="timeline-track">
                <div className="timeline-node" style={{ background: color, borderColor: color }}>
                  <Icon size={9} color="white" />
                </div>
                <div className="timeline-line" style={{ background: color + '30' }} />
              </div>
              ...
              <div className="timeline-event-content">
                <div className="timeline-event-top">
                  <div className="timeline-event-left">
                    ...
                    {event.severity && (
                      <span className={`badge badge-${event.severity === 'high' ? 'red' : event.severity === 'medium' ? 'yellow' : 'gray'}`}>
                        {event.severity}
                      </span>
                    )}
                  </div>
